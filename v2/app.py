...
def orders():
    conn = get_db()
    user_orders = conn.execute("SELECT o.order_id, o.price, o.order_status as status, o.created_at, o.hashed_otp, i.name as item_name FROM orders o JOIN items i ON o.item_id = i.item_id WHERE o.buyer_id = ? ORDER BY o.created_at DESC",(session["user_id"],)).fetchall()
    conn.close()
...
def place_order():
    ...
    if not cart or not isinstance(cart, list): return jsonify({"ok": False, "error": "Cart is invalid or empty"}), 400
    conn = get_db()
    try:
        with conn:
            cur = conn.cursor()
            for item in cart:
                ...
                item_data = cur.execute("SELECT seller_id, price, quantity FROM items WHERE item_id = ? AND status = 'available'", (item_id,)).fetchone()
                if not item_data or item_data["quantity"] < quantity_ordered: raise ValueError(f"'{item.get('name')}' is out of stock or unavailable.")
                order_id = generate_order_id()
                _otp, hashed_otp = generate_hashed_otp()
                seller_id = item_data["seller_id"]
                price_at_sale = item_data["price"] * quantity_ordered
                cur.execute("INSERT INTO orders (order_id, buyer_id, seller_id, item_id, price, quantity_ordered, payment_mode, hashed_otp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (order_id, buyer_id, seller_id, item_id, price_at_sale, quantity_ordered, payment_mode, hashed_otp))
